      ...
      if (match) {
        ...
        if (match[3] && match[3] !== "0") {
          ...
          switch (match[2]) {
            ...
            break;
            default:
              throw new Error('Unsupported param ' + match[1] + ' while parsing google image service command ' + match[0])
          }
        } else {
          // when /:/gis=s0, /:/gis, we need to drop gis from the path.
          replacement = ``
        }  
        routeInfo.urlInfo.pathname = routeInfo.urlInfo.pathname.replace(match[0], replacement)
      }
    },
    steps: {
      fm: {

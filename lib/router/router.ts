...
import defaults from './router-defaults.ts';
...
export default class Router extends EventEmitter {
  ...
  getInfo(req: any, opts?: any): any {
    ...
    const signatureParts = routeInfo.urlInfo.pathname.split(
    ...
    const pathParts = routeInfo.urlInfo.pathname
    ...
    routeInfo.imageSteps = this.#getImageStepsFromRoute(pathParts[1]);
    ...
    if (!routeInfo.flatSteps.format) {
      ...
      if (
        ...
        const fmt = useAVIF ? 'avif' : useWebP ? 'webp' : fallbackFormat;
  ...
  getStepsFromObject(obj: any, options: any) {
    const steps: any[] = [];
    ...
    Object.keys(obj).forEach((key) => {
      ...
      const val = _.merge({ name: key }, obj[key]);
  ...
  #getImageStepsFromRoute(imageStepsStr?: string): any[] {
    if (!imageStepsStr) return [];
    const imageSteps = imageStepsStr.split(this.options.cmdKeyDelimiter);

    return imageSteps.map((stepStr) => {
      ...
      const stepParams = stepParts[1].split(this.options.paramKeyDelimiter);
      stepParams.forEach((stepParam) => {
        const paramParts = stepParam.split(this.options.paramValDelimiter);
        const paramName = paramParts[0];
        const fullParamName = config[paramName];
        if (!fullParamName) {
          throw new Error(
...
function getContentType(req: any, routeInfo: any): string | undefined {
  const contentType = mime.getType(routeInfo.originalPath);
  if (
...
function flattenSteps(steps: any[]) {

...
p.getInfo = function(req) {
  ...
  routeInfo.urlInfo.pathname = routeInfo.urlInfo.pathname.replace(/\%20/g, ' ');
  ...
  if (routeInfo.imageSteps.filter(filterRotate).length === 0) {
    ...
    routeInfo.imageSteps.push({ name: 'rotate', degrees: 'auto' });
  }
...
function getContentType(req, routeInfo) {
  ...
  for (var i = 0; i < otherContentTypeKeys.length; i++) {
    ...
    if (ctInfo.pathRegEx.test(routeInfo.urlInfo.pathname) && ctInfo.acceptRegEx.test(acceptsHeader)) {

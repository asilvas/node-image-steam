...
p.getInfo = function(req) {
  ...
  if (routeInfo.imageSteps.length === 0 && ((this.options.ifNoOperation && this.options.ifNoOperation.justForward === true) || routeInfo.urlInfo.query.noop === 'true')) {
    var imageTypeIfUnknown;
    if (this.options.ifNoOperation) imageTypeIfUnknown = this.options.ifNoOperation.imageTypeIfUnknown;
    contentType = getImageContentType(req,routeInfo,imageTypeIfUnknown);
    if (contentType) {

...
function Router(options) {
  ...
  if (typeof this.options.originalSteps === 'object') {
    ...
    this.hashFromOptimizedOriginal = helpers.imageSteps.getHashFromSteps(
      this.originalSteps
    );
  }

  if (typeof this.options.hqOriginalSteps === 'object') {
    this.hqOriginalSteps = this.getStepsFromObject(
      this.options.hqOriginalSteps
    );
  }
}
...
p.getInfo = function (req) {
  ...
  var signatureParts = routeInfo.urlInfo.pathname.split(
    this.options.signatureDelimiter
  );
  ...
  if (typeof this.options.beforeProcess === 'function') {
    this.options.beforeProcess(routeInfo, this.options);
  }

  // break apart imagePath from imageSteps from queryParams
  var pathParts = routeInfo.urlInfo.pathname
    .split(this.options.signatureDelimiter)[0]
    .split(this.options.pathDelimiter);
  ...
  const useOriginal =
    this.options.useOriginal || routeInfo.urlInfo.query.useOriginal === 'true';

...
module.exports = function (context, stepInfo) {
  ...
  var anchorX = isNaN(stepInfo.anchorX)
      ? Math.floor(img.info.width / 2)
      : stepInfo.anchorX,
    anchorY = isNaN(stepInfo.anchorY)
      ? Math.floor(img.info.height / 2)
      : stepInfo.anchorY;
  ...
  if (
    context.saliency &&
    context.originalImage.info.saliency &&
    (stepInfo.anchor === 'auto' ||
      (!stepInfo.anchor && context.saliency.options.autoCrop))
  ) {
    const region = context.saliency.getSaliencyRegion(
      context.originalImage.info.saliency,
      {
        regionWidth: stepInfo.width,
        regionHeight: stepInfo.height,
        imageWidth: img.info.width,
        imageHeight: img.info.height,
      }
    );

    stepInfo.top = region.top;
    stepInfo.left = region.left;
    stepInfo.width = region.width;
    stepInfo.height = region.height;
  }

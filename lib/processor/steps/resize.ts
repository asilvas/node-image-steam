...
export default function resize(context: any, stepInfo: any) {
  ...
  const aspectX = helpers.dimension.getXAspect(img);
  const aspectY = helpers.dimension.getYAspect(img);
  ...
  const ignoreAspect = stepInfo.ignoreAspect === 'true';

  if (!ignoreAspect) {
    let w = stepInfo.width,
      h = stepInfo.height;
  ...
  const rgba = helpers.rgba.getRGBA(stepInfo);
  context.sharp.resize(stepInfo.width, stepInfo.height, {

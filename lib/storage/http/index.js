...
p.fetch = function(options, originalPath, stepsHash, cb) {
  ...
  client.request(reqOptions, function(res) {
    ...
    res.on('end', function() {
      ...
      var meta = {};
      try {
        meta = JSON.parse(res.headers['x-isteam-meta'] || res.headers['x-amz-meta-isteam'] || '{}');
      } catch (ex) {
...
p.getRequestOptions = function(pathInfo, options) {
  ...
  const postfix = options.isteamEndpoint ? options.useOriginal
    ? '?useOriginal=true'
    : `${options.pathDelimiter || '/:/'}fm=f:${options.format || 'webp'}`
    : ''
  ;
  ...
  return {
    ...
    port: urlInfo.port ? parseInt(urlInfo.port) : urlInfo.protocol === 'https:' ? 443 : 80,
    path: `${urlInfo.path}${sep}${bucketPath}${encodeURI(pathInfo.imagePath)}${postfix}`,
    method: 'GET',
    headers
  };
};

...
p.fetch = function(options, originalPath, stepsHash, cb) {
  if (stepsHash) return void cb(new Error('HTTP storage driver does not support writes')); // no image if stepsHash is requested
...
function getImageUri(pathInfo, options) {
  return options.endpoint + '/' + pathInfo.bucket + '/' + encodeURI(pathInfo.imagePath);
};

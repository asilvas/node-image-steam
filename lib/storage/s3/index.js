...
function StorageS3(options) {
  ...
  assert(this.options.accessKey, 'storage.s3.accessKey required');
  assert(this.options.secretKey, 'storage.s3.secretKey required');
}
...
p.fetch = function(originalPath, stepsHash, cb) {
  ...
  var client = this.getClient(pathInfo.bucket);

...
p.deleteCache = function(req, originalPath, cb) {
  var $this = this;
  var reqInfo;
  try {
    reqInfo = this.getRequestInfo(originalPath, req, 'cache');
  } catch (ex) {
    this.emit('warn', ex);
    return void cb(ex);
  }
  if (!reqInfo.driver.deleteCache) {
    const err = new Error(`deleteCache not supported on storage driver ${reqInfo.driver.name}`);
    this.emit('warn', err);
    return void cb(err);
  }
  reqInfo.driver.deleteCache(reqInfo.options, reqInfo.realPath, function(err) {
    if (err) {
      $this.emit('warn', err);
      return void cb(err);
    }

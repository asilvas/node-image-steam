...
p.getDriver = function(options, prefix) {
  const name = options.driverPath ? `${prefix}:byPath/${options.driverPath}` : `${prefix}:byName/${options.driver}`;
...
p.fetch = function(req, { originalPath, hashFromOptimizedOriginal, urlInfo }, { hash }, cb) {
  ...
  driverInfo.driver.fetch({ useOriginal, ...driverInfo.options }, driverInfo.realPath, hash, function(err, img, imgData) {
    if (err) {
      $this.emit('warn', err);
      return void cb(err);
    }

...
p.fetch = function(req, originalPath, stepsHash, cb) {
  var $this = this;
  var reqInfo = this.getRequestInfo(originalPath, req);
  this.driver.fetch(reqInfo.options, reqInfo.realPath, stepsHash, function(err, img) {
    if (err) {
      $this.emit('warn', err);
      return void cb(err);
    }
...
p.store = function(req, originalPath, stepsHash, image, cb) {
  var $this = this;
  var reqInfo = this.getRequestInfo(originalPath, req);
  this.driver.store(reqInfo.options, reqInfo.realPath, stepsHash, image, function(err) {
    if (err) {
      $this.emit('warn', err);
      return void cb(err);
    }
...
p.getRequestInfo = function(originalPath, req) {

...
function startServer(config, httpConfig) {
  if (httpConfig.ssl) {
    ...
    if (!('allowHTTP1' in httpConfig.ssl)) httpConfig.ssl.allowHTTP1 = true; // default
  }
  ...
  var server =
      httpConfig.ssl ?
        http2.createSecureServer(httpConfig.ssl, processRequest.getHandler())
        :
        http2.createServer(processRequest.getHandler())
    ;

  ...
  return function(req, res) {
    ...
    async.auto({
      ...
      requestedImage: ['getPrefetcher', function(cb) {
        ...
        $this.storage.fetch(reqInfo.originalPath, reqInfo.hashFromSteps, function(err, image) {
          if (err) {
            return void cb();
          }
      ...
      optimizedOriginalImage: ['requestedImage', function(cb, results) {
        ...
        if (!$this.router.hashFromOptimizedOriginal) {
          // no optimized original steps, feature disabled
          return void cb();
        }
        if (reqInfo.hashFromSteps === $this.router.hashFromOptimizedOriginal) {
        ...
        $this.storage.fetch(reqInfo.originalPath, $this.router.hashFromOptimizedOriginal, function(err, image) {
          if (err) {
            return void cb();
          }
      ...
      originalImage: ['optimizedOriginalImage', function(cb, results) {
        ...
        $this.storage.fetch(reqInfo.originalPath, null, function(err, image) {
          if (err) {
            return void cb(err);
          }
      ...
      processedOptimizedImage: ['hasProcessor', 'optimizedOriginalImage', 'originalImage', function(cb, results) {
        ...
        $this.processor.process(results.originalImage, $this.router.optimizedOriginalSteps, cb);
      }],
      ...
      storeProcessedImage: ['processedImage', function(cb, results) {
        ...
        $this.storage.store(reqInfo.originalPath, reqInfo.hashFromSteps, results.processedImage, function(err) {
      ...
      storeOptimizedImage: ['processedOptimizedImage', function(cb, results) {
        ...
        // store processed image
        $this.storage.store(reqInfo.originalPath, $this.router.hashFromOptimizedOriginal, results.processedOptimizedImage, function(err) {
          // ignore, error bubbled up through events
        });
      }]

...
function Connect(options) {
  ...
  if (this.saliency.options.enabled) {
    console.warn(
      '!!! Saliency has been deprecated and will be removed in future version !!!'
    );
    ...
    async.auto(
      {
        ...
        requestedImage: [
          'getPrefetcher',
          async.timeout((results, cb) => {
            if (!reqInfo.hashFromSteps || reqInfo.optimized) {
        ...
        refreshImageArtifact: [
          ...
          async.timeout((results, cb) => {
            ...
            const deltaS =
              (Date.now() - image.info.lastModified.valueOf()) / 1000;
            if (deltaS < tts) {
              // not yet stale
              return void cb();
            }
        ...
        processedOptimizedImage: [
          ...
          async.timeout((results, cb) => {
            ...
            if (!reqInfo.isCachable) {
              // if skipCache is provided, do not create optimized
              return void cb();
            }
        ...
        saliency: [
          ...
          async.timeout((results, cb) => {
            ...
            if (
              img.info.saliency &&
              img.info.saliency.v === this.saliency.autoFocus.constants.VERSION
            )
              return void cb(); // already has saliency

            img.info.saliency = null; // reset saliency if set

            const crop = reqInfo.imageSteps.filter(
              (step) => step.name === 'crop'
            )[0];
              ...
              (crop &&
                (crop.anchor === 'auto' ||
                  (!crop.anchor &&
                    !crop.anchorX &&
                    !crop.anchorY &&
                    this.saliency.options.autoCrop))) ||
              // if saliency command then we need it
              (reqInfo.command &&
                (reqInfo.command.name === 'saliency' ||
                  reqInfo.command.name === 'saliencyMap'));
        ...
        processedImage: [
          ...
          'saliency',
          async.timeout((results, cb) => {
            if (reqInfo.command || reqInfo.useOriginal || reqInfo.optimized) {
              return void cb();
            }
            if (!results.hasProcessor) {
              return void cb();
        ...
        storeProcessedImage: [
          ...
          async.timeout((results, cb) => {
            ...
            results.processedImage.info.author =
        ...
        storeOptimizedImage: [
          ...
          async.timeout((results, cb) => {
            ...
            // track address of author
            results.processedOptimizedImage.info.author =
              req.headers['x-forwarded-for'] || req.connection.remoteAddress;

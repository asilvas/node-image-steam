...
p.getHandler = function() {
  ...
  return function(req, res) {
    ...
    if (req.method === 'OPTIONS') {
      ...
      if (reqInfo.originalPath.indexOf('.') < 0) { // extension is required otherwise may be malicous to purge directories
        res.writeHead(400);
        return void res.end();
      }
    ...
    async.auto({
      ...
      optimizedOriginalImage: ['requestedImage', async.timeout(function optimizedOriginalImage(results, cb) {
        ...
        $this.storage.fetch(req, reqInfo.originalPath, reqInfo.hashFromOptimizedOriginal, function(err, image) {
          ...
          if (image.info.byteSize !== image.buffer.length) {
            $this.emit('warn', 'optimizedOriginal.byteSize !== buffer.length may indicate a problem with storage driver');
            return void cb();
          }

...
function Connect(options) {
  ...
  if (this.saliency.options.enabled) {
    console.warn('!!! Saliency has been deprecated and will be removed in future version !!!');
  ...
  return (req, res) => {
    ...
    async.auto({
      ...
      hasProcessor: ['checkContentType','requestedImage', 'optimizedOriginalImage', 'originalImage', async.timeout((results, cb) => {
        ...
        if (results.requestedImage || (results.optimizedOriginalImage && reqInfo.optimized === true)) {
      ...
      saliency: ['optimizedOriginalImage', 'processedOptimizedImage', 'originalImage', async.timeout((results, cb) => {
        ...
        if (img.info.saliency && img.info.saliency.v === this.saliency.autoFocus.constants.VERSION) return void cb(); // already has saliency
      ...
      const image = results.requestedImage
          ...
          || (reqInfo.optimized === true && (results.optimizedOriginalImage || results.processedOptimizedImage))
      ...
      if (!isNaN(image.info.orientation)) {
        ...
        headers['x-isteam-meta'] = JSON.stringify({ orientation: image.info.orientation });
      }

  ...
  return (req, res) => {
    ...
    async.auto({
      ...
      saliency: ['optimizedOriginalImage', 'processedOptimizedImage', 'originalImage', async.timeout((results, cb) => {
        if (!this.saliency.options.enabled) return void cb(); // not enabled
        ...
        if (img.info.saliency && img.info.saliency.v === this.saliency.options.version) return void cb(); // already has saliency
        ...
        const needSaliency =
          ...
          reqInfo.command && reqInfo.command.name === 'saliency'
        ;
        ...
        this.saliency.getSaliencyMeta(img, {}, (err, meta) => {
          ...
          // force version
          img.info.saliency.v = this.saliency.options.version;

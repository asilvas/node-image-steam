...
var imageType = require('../helpers/image-type');
...
p.getHandler = function() {
  ...
  return function(req, res) {
    ...
    async.auto({
      ...
      checkContentType: ['requestedImage', async.timeout(function checkContentType(results,cb){
        if (reqInfo.useOriginal) {
          helpers.imageType(req,reqInfo.originalPath,reqInfo.guestType,function(err,contentType){
            if (err) return void cb(err);
            //TODO: Improve it to be able to convert to a compatible format if contentType is null
      ...
      optimizedOriginalImage: ['checkContentType','requestedImage', async.timeout(function optimizedOriginalImage(results, cb) {
      ...
      var contentType = reqInfo.contentType || image.contentType

      if (contentType !== 'unknown') headers['Content-Type'] = contentType;

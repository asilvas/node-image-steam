  ...
  return function(req, res) {
    ...
    async.auto({
      ...
      checkContentType: ['requestedImage', async.timeout(function checkContentType(results,cb){
        if (reqInfo.contentType === 'unknown') {
          // try to detect content checking image file hearders
          reqInfo.contentType = imageType(reqInfo.originalPath);
        } else {
          // just skip if content type is known
          return void cb();
        }
      }, $this.options.stepTimeout)],

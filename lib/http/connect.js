  ...
  return function(req, res) {
    ...
    async.auto({
      ...
      optimizedOriginalImage: ['requestedImage', function(cb, results) {
        ...
        $this.storage.fetch(req, reqInfo.originalPath, $this.router.hashFromOptimizedOriginal, function(err, image) {
          if (err) {
            return void cb();
          }
      ...
      storeOptimizedImage: ['processedOptimizedImage', function(cb, results) {
        ...
        // store processed image
        $this.storage.store(req, reqInfo.originalPath, $this.router.hashFromOptimizedOriginal, results.processedOptimizedImage, function(err) {
          // ignore, error bubbled up through events
        });
      }]

...
p.getSaliencyMeta = function (image, options, cb) {
  this.getSaliencyMap(image, options, (err, saliencyMap) => {
    ...
    const meta = this.autoFocus.getMetaFromSalientMatrix(
      saliencyMap.getDataAsArray()
    );
...
p.getSaliencyMapAsImage = function (image, options, cb) {
  this.getSaliencyMap(image, options, (err, saliencyMap) => {
    ...
    if (options.autoFocus && meta) {
      ...
      meta.r25th &&
        finalImage.drawRectangle(
          new this.cv.Rect(
            meta.r25th.l * finalImage.cols,
            meta.r25th.t * finalImage.rows,
            meta.r25th.w * finalImage.cols,
            meta.r25th.h * finalImage.rows
          ),
          new cv.Vec(100, 100, 255)
        );
      meta.r40th &&
        finalImage.drawRectangle(
          new this.cv.Rect(
            meta.r40th.l * finalImage.cols,
            meta.r40th.t * finalImage.rows,
            meta.r40th.w * finalImage.cols,
            meta.r40th.h * finalImage.rows
          ),
          new cv.Vec(150, 150, 255)
        );
      meta.r50th &&
        finalImage.drawRectangle(
          new this.cv.Rect(
            meta.r50th.l * finalImage.cols,
            meta.r50th.t * finalImage.rows,
            meta.r50th.w * finalImage.cols,
            meta.r50th.h * finalImage.rows
          ),
          new cv.Vec(0, 0, 255)
        );
      meta.r75th &&
        finalImage.drawRectangle(
          new this.cv.Rect(
            meta.r75th.l * finalImage.cols,
            meta.r75th.t * finalImage.rows,
            meta.r75th.w * finalImage.cols,
            meta.r75th.h * finalImage.rows
          ),
      ...
      meta.r90th &&
        finalImage.drawRectangle(
          new this.cv.Rect(
            meta.r90th.l * finalImage.cols,
            meta.r90th.t * finalImage.rows,
            meta.r90th.w * finalImage.cols,
            meta.r90th.h * finalImage.rows
          ),
          new cv.Vec(0, 255, 0)
        );
    }

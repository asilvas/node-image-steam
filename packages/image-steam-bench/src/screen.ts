...
export default class Screen {
  ...
  grid: any;
  screenLog: any;
  screenThroughput: any;
  screenLatency: any;
  screenErrorsConcurrency: any;
  screenScores: any;
  timeLabels: string[];
  throughputData: any;
  latencyData50th: any;
  ...
  constructor(bench: any) {
    this.bench = bench;

    if (this.bench.argv.log !== 'false')
      writeFileSync(this.bench.argv.log, 'Started\n');
    ...
    this.timeLabels = Array.from(Array(this.bench.argv.timeWindow)).map(
      (v, i) => ` `
    );
  ...
  log(msg: any, type = 'log') {
    ...
    if (this.bench.argv.log !== 'false')
      appendFileSync(this.bench.argv.log, line + '\n');
  ...
  get scores() {
    return this.bench.scores;
  }
  get testData() {
    return this.bench.testData;
  }
  ...
  render() {
    ...
    this.screenErrorsConcurrency.setData([
      this.errorsData,
      this.concurrencyData,
    ]);
  ...
  getScoreData() {
    ...
    return this.bench.argv.test.map((testName: string, idx: number) => {
      const scores = this.scores[testName];

      if (scores && scores.max.rps) this.scoreIndex = idx + 1;

      const state =
        scores && scores.max.rps
          ? 'complete'
          : this.bench.testName === testName
          ? 'working'
          : 'pending';

      const perf =
        scores && scores.perf.ttfb50th
          ? `${scores.perf.ttfb50th}ms, ${scores.perf.ttfb75th}ms, ${
              scores.perf.ttfb90th
            }ms, ${kbpsToString(scores.perf.kbps)}`
          : '';
      const minLoad =
        scores && scores.min.rps
          ? `${scores.min.rps}rps, ${scores.min.concurrency}cc, ${
              scores.min.ttfb
            }ms, ${kbpsToString(scores.min.kbps)}`
          : '';
      const optimalLoad =
        scores && scores.optimal.rps
          ? `${scores.optimal.rps}rps, ${scores.optimal.concurrency}cc, ${
              scores.optimal.ttfb
            }ms, ${kbpsToString(scores.optimal.kbps)}`
          : '';
  ...
  getScoreMarkup(): string[] {
    return this.getScoreData().reduce(
  ...
  printScoreDataAsMarkup() {
    ...
    this.getScoreMarkup().forEach((line: string) => console.log(line));
  }
...
function kbpsToString(kbps: number) {

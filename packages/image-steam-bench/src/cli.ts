...
import tests from './test/index.ts';
...
yargs(hideBin(process.argv))
  .option('port', {
    ...
    type: 'number',
    describe:
      'Port for image-steam-bench to listen on (same port image-steam should be mapped back to)',
    default: 12124,
  })
  .option('test', {
    alias: 't',
  ...
  .option('minLoad', {
    type: 'number',
    describe:
      'Increase in 50th response times before considered minimum safe load',
    default: 1.25,
  })
  .option('maxLoad', {
    type: 'number',
  ...
  .option('minRunTime', {
    type: 'number',
    describe:
      'Minimum time (in ms) that a test must run before determinating. If `requests` is specified that will take priority',
    default: 15000,
  })
  .option('requests', {
    type: 'number',
    describe:
      'A fixed number of requests before resolving test(s), versus the default behavior of ending on `maxLoad`',
  })
  .option('workerMin', {
    type: 'number',
  ...
  .option('workerSpawnRate', {
    type: 'number',
    describe:
      'The rate at which workers are spawned (0.2 being +20% per spawn)',
    default: 0.25,
  })

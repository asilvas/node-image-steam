...
export default class Bench {
  ...
  onTestData({ workerIndex }: any, { requests, errors }: any) {
    this.testData.lastTickRequests =
      this.testData.lastTickRequests.concat(requests);
  ...
  updateTestStats() {
    ...
    if (!this.testData.isOver && this.testData.lastTickRequests.length < 8)
      return;

    this.testData.errors += this.testData.lastTickErrors;

    const ttfbSorted = this.testData.lastTickRequests.sort((a: any, b: any) =>
      a.ttfb < b.ttfb ? -1 : 1
    );
    const ttfb50th = (this.ttfb50th =
      ttfbSorted[Math.floor(ttfbSorted.length / 2)].ttfb);
    const ttfb75th = (this.ttfb75th =
      ttfbSorted[Math.floor(ttfbSorted.length * 0.75)].ttfb);
    const ttfb90th = (this.ttfb90th =
      ttfbSorted[Math.floor(ttfbSorted.length * 0.9)].ttfb);
    const elapsed = Date.now() - this.testData.lastUpdate;
    const rpsFactor = 1000 / elapsed;
    const rps = (this.rps = Math.round(
      this.testData.lastTickRequests.length * rpsFactor
    ));
    this.eps = Math.round(this.testData.lastTickErrors * rpsFactor);
    const totalKb = ttfbSorted.reduce((total: number, { size }: any) => {
    ...
    this.testData.requests = this.testData.requests.concat(
      this.testData.lastTickRequests
    );
    ...
    if (
      !this.testData.score.min.ttfb &&
      ttfb50th >= this.testData.score.perf.ttfb50th * this.argv.minLoad
    ) {
    ...
    if (
      !this.testData.score.optimal.rps ||
      rps > this.testData.score.optimal.rps
    ) {
      ...
      !this.argv.requests &&
      timeSinceLastOptimal > this.argv.minRunTime
    ) {
    ...
    const maxLoadLatency = Math.max(
      20,
      Math.floor(this.testData.score.optimal.ttfb * this.argv.maxLoad)
    );
    ...
    if (
      ...
      if (!this.testData.score.min.ttfb) {
        // if no minimum yet set, do it now
        this.testData.score.min.ttfb = ttfb50th;
        this.testData.score.min.rps = rps;
        this.testData.score.min.concurrency = this.concurrency;
        this.testData.score.min.kbps = kbps;
    ...
    if (
      !this.argv.requests &&
      timeSinceStart > this.argv.minRunTime &&
      !this.testData.score.max.ttfb &&
      ttfb50th >= maxLoadLatency
    ) {
